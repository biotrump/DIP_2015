//http://blog.csdn.net/byxdaz/article/details/5642669
...
int main(int argc, char* argv[])
{
	...
	cvZero(pDst);
	cvThreshold(pSrc,pTmp,128,1,CV_THRESH_BINARY_INV);//binary image
	...
	for(i=0; i<size.height;  i++)
	{
		for(j=0; j<size.width; j++)
		{
			if(CV_IMAGE_ELEM(pDst,uchar,i,j)==1)//toggle 1 to 0
			{
				CV_IMAGE_ELEM(pDst,uchar,i,j) = 0;
			}
			else
			{
				CV_IMAGE_ELEM(pDst,uchar,i,j) = 255;//toggle 0 to 255
			}
		}
	}
